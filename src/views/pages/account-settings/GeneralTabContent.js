...
import difultImg from "../../../assets/images/avatars/default.PNG"

const GeneralTabs = ({ data }) => {
  const [email, setEmail] = useState(data.email ? data.email : '')
  const [name, setName] = useState(data.fullName ? data.fullName : '')
  const [avatar, setAvatar] = useState(data.avatar ? data.avatar : difultImg)
    ...
    <Fragment>
      <Media className="align-items-center">
        <Media className='mr-25' left>
          <Media object className='rounded mr-50 ' src={avatar} alt='Generic placeholder image' width='200'/>
        </Media>
